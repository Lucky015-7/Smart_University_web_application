...
interface ViewTicketsClientProps {
  initialOpenTickets: TicketData[]
  initialOtherTickets: TicketData[]
}

export default function ViewTicketsClient({
  initialOpenTickets,
  initialOtherTickets,
}: ViewTicketsClientProps) {
  const [selectedTab, setSelectedTab] = useState<"open" | "others">("open")
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedCategory, setSelectedCategory] = useState("all")
  const [selectedPriority, setSelectedPriority] = useState("all")
  const [page, setPage] = useState(1)

  const [openTickets, setOpenTickets] = useState<TicketData[]>(initialOpenTickets)
  const [otherTickets, setOtherTickets] = useState<TicketData[]>(initialOtherTickets)
  ...
  const fetchAllTickets = useCallback(async (): Promise<TicketData[]> => {
    const limit = 200
    let currentPage = 1
    let totalPages = 1
    ...
    do {
      const response = await fetch(`/api/tickets?page=${currentPage}&limit=${limit}`)
      if (!response.ok) {
  ...
  const fetchTickets = useCallback(
    async () => {
      ...
      try {
        ...
        const open = filtered.filter((ticket) => ticket.status === "OPEN")
        const others = filtered.filter((ticket) => ticket.status !== "OPEN")
    ...
    [fetchAllTickets, searchQuery, selectedCategory, selectedPriority]
  )
  ...
  }, [searchQuery, selectedCategory, selectedPriority, fetchTickets])

  const handlePageChange = (newPage: number) => {
    setPage(newPage)
  }

  const currentTickets = selectedTab === "open" ? openTickets : otherTickets
  const totalPages = Math.ceil(currentTickets.length / LIMIT)
      ...
      <div className="container  mx-auto py-8 px-4">
        <div className="space-y-2 mb-8">
          ...
          <p className="text-muted-foreground">
            Manage all incident tickets, view details, and track resolution
            progress
          </p>
        </div>
        ...
        <Tabs value={selectedTab} onValueChange={(value) => setSelectedTab(value as "open" | "others")}>
          <TabsList className="grid w-full grid-cols-2 mb-6">
            ...
            <TabsTrigger value="others" className="gap-2 flex items-center">
              <CheckCircle className="h-4 w-4" />
              <span>
                Other Tickets
                {!isLoading && otherTickets.length > 0 && (
                  <span className="ml-2 inline-flex items-center gap-1 bg-blue-500/10 text-blue-700 dark:text-blue-400 px-2 py-1 rounded text-xs font-semibold">
                    {otherTickets.length}
                  </span>
                )}
              </span>
              ...
              <>
                <div className="grid gap-4">
                  {paginatedTickets.map((ticket) => (
                    <AdminTicketRow key={ticket.id} ticket={ticket} onSelect={() => {}} />
                  ))}
                </div>
          ...
          <TabsContent value="others" className="space-y-4">
            {isLoading ? (
              ...
              <Card className="p-8 border-dashed text-center">
                ...
                <p className="text-lg font-medium">No Other Tickets</p>
                <p className="text-muted-foreground mt-2">
                  Tickets with status IN_PROGRESS, RESOLVED, CLOSED, or REJECTED will appear here.
                </p>
              </Card>
            )}
          </TabsContent>
