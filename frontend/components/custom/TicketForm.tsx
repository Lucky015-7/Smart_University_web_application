...
const ticketSchema = z.object({
    ...
    category: z.enum(["OTHER", "ELECTRICAL", "PLUMBING", "SOFTWARE", "HARDWARE"], {
...
export const TicketForm = ({ resourceId }: { resourceId: string }) => {
    ...
    const onSubmit = async (values: TicketFormData) => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : "Error creating a ticket";
            toast.error(message);
            console.error(error);
        }
    }
    ...
    return (
        <main>
            ...
            <FieldGroup>
                <FieldSet>
                    <FieldLegend>Incident Report Form</FieldLegend>
                    <FieldDescription>
                        Please provide the details of the incident below.
                    </FieldDescription>
                </FieldSet>
                <Field>
                    <FieldLabel htmlFor="ticket_attachments">
                        Attach Images
                    </FieldLabel>
                    {/* Image upload */}
                    <ImageUpload
